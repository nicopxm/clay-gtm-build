...
- 2026-08-01: **Clay sprint COMPLETE — supersedes the 2026-07-17 "NOT YET DONE" entry below, all four items now shipped.** Step 1 (posting-body scrape + re-score, 2026-07-21): 8 gate-passers scraped at 1.5 cr; 5 of 8 postings had closed since seed collection; Hologram moved rank 8→2. Step 2 (contact waterfall, 2026-07-22): Surfe → Clay escalation → Icypeas, 1.1 cr, 2 contacts at `ultra_sure`. Step 3 (HubSpot push, 2026-07-27): 2 Contacts live, 0 credits. Step 4 (benchmark, 2026-07-29→30): 8 accounts × 3 n8n sub-workflows, 0 credits. Final ledger 1,914.3 — 90.7 of 2,005 trial credits spent. Trial expired 2026-07-29 as forecast; all Clay-side work completed before expiry.
  - **§3 amendment:** "`icp_score` property created in HubSpot — but never written to" is now FALSE. Clay wrote the first real values to that property (Hyperbound 68.75, Hologram 54.75), verified in HubSpot's Contact properties view. Pipeline-side scoring (#29) remains unbuilt — the property now holds Clay-sourced values only.
  - **Dedupe, partial:** the explicit lookup-first pattern mirroring #24 was verified on Hyperbound (Create → force re-run → Update, single record confirmed). Hologram was pushed via a simplified gate after Clay's chained-action reference resolution failed; no duplicate created, but that row relied on HubSpot's server-side email dedupe rather than the explicit check. Recorded as 1-of-2 clean, not as a clean pass.
  - **§2 benchmark rule, satisfied by construction:** the 3 benchmarked sub-workflows (website scraper, tech-stack fingerprint, news RSS) make no Claude API calls, verified by reading each node list. The n8n cost figure is therefore a confirmed $0.00, not an ESTIMATE needing a label. The rule was not waived — it did not apply.
  - **Pipeline state UNCHANGED by this sprint:** #23 orchestration still not wired; #29 scoring still not built; no lead has a pipeline-produced `icp_score`. Nothing in the Clay build alters §1, §2, or §4.
